...
export default function Reports() {
  ...
  const generatePDF = async () => {
    ...
    try {
      // Fetch fitting rooms data
      let rooms: { name: string; roomId: string; branchCode: string; status: string; location: string }[] = [];
      if (branch === "ALL") {
        // Fetch rooms from every branch in parallel
        const allRooms = await Promise.all(
          branches.map(b =>
            fetch(`${API_BASE}/fitting-rooms?branchCode=${encodeURIComponent(b)}`, { headers: authHeaders() })
              .then(r => r.ok ? r.json() : [])
          )
        );
        rooms = allRooms.flat();
      ...
      for (const rt of REPORT_TYPES) {
        ...
        if (rt.name === "Fitting Room Stats") {
          ...
          head = [cols];
          // Group rooms by branch code
          const branchGroups = new Map<string, typeof rooms>();
          for (const r of rooms) {
            const bc = r.branchCode ?? branch;
            if (!branchGroups.has(bc)) branchGroups.set(bc, []);
            branchGroups.get(bc)!.push(r);
          }
          if (branchGroups.size === 0) branchGroups.set(branch || "—", []);
          body = [...branchGroups.entries()].map(([bc, bRooms]) => {
            const row = [
              bc,
              String(bRooms.length),
